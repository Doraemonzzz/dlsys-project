...
class NDArray:
    ...
    def __getitem__(self, idxs):
        ...
        for i in range(self.ndim):
            new_shape[i] = (idxs[i].stop - idxs[i].start) // idxs[i].step
            # add
        ...
        new_offset = 0
        for i in range(self.ndim):
            new_offset += self.strides[i] * idxs[i].start
...
def max(a, axis=None, keepdims=False):
    # res = a.max(axis, keepdims)
    # if axis == None:
    #     return NDArray(res, device=res.device)
    # else:
    #     return res
...
def make(a, shape, strides=None, offset=0):
    return NDArray.make(
        shape,
        strides,
        a._device,
